...
class CrossMambaBlock(nn.Module):
    ...
    def forward(self, x_rgb, x_thermal):
        ...
        attn_out, _ = self.cross_attn(x_rgb, x_thermal, x_thermal)
        x_rgb = x_rgb + attn_out
        
        B, H, W, C = x_rgb.shape
        L = H * W

        
        x_rgb = x_rgb.reshape(B, L, C)
        x_thermal = x_thermal.reshape(B, L, C)

        
        delta_rgb = torch.exp(self.proj_delta(x_rgb))
        delta_thermal = torch.exp(self.proj_delta(x_thermal))
        B_rgb = self.proj_B(x_rgb)
        B_thermal = self.proj_B(x_thermal)
        C_rgb = self.proj_C(x_rgb)
        C_thermal = self.proj_C(x_thermal)

        
        h_rgb = delta_rgb * torch.einsum('bln,bld->bnd', B_rgb, x_rgb)
        h_thermal = delta_thermal * torch.einsum('bln,bld->bnd', B_thermal, x_thermal)

       
        y_rgb = torch.einsum('bnd,bnd->bd', h_rgb, C_thermal)
        y_thermal = torch.einsum('bnd,bnd->bd', h_thermal, C_rgb)

        return y_rgb.view(B, H, W, C), y_thermal.view(B, H, W, C)


class ConcatMambaBlock(nn.Module):
    """ ConMB  """
    ...
    def forward(self, x1, x2):
        ...
        seqs = []
        for dim in [1, 2]:  
            seq_forward = selective_scan_fn(
                x_concat.flatten(1, 2),
                torch.zeros(B, H * W, self.d_state).to(x1.device),
                torch.randn(self.d_state).to(x1.device),
                torch.randn(self.d_state).to(x1.device),
                torch.zeros(self.d_state).to(x1.device)
            )
            seq_backward = selective_scan_fn(
                x_concat.flip(dims=[dim]).flatten(1, 2),
                torch.zeros(B, H * W, self.d_state).to(x1.device),
                torch.randn(self.d_state).to(x1.device),
                torch.randn(self.d_state).to(x1.device),
                torch.zeros(self.d_state).to(x1.device)
            )
            seqs.append(seq_forward + seq_backward.flip(dims=[1]))

        fused = sum(seqs).view(B, H, W, -1)
        
        gate_val = self.gate(fused)
        fused = gate_val * fused + (1 - gate_val) * x1
        return self.proj(fused)



class ChannelAwareVSSBlock(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        self.vss = VSSBlock_new(hidden_dim=d_model)
        self.channel_attn = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.AdaptiveMaxPool2d(1),
            nn.Conv2d(d_model, d_model // 8, 1),
            nn.ReLU(),
            nn.Conv2d(d_model // 8, d_model, 1),
            nn.Sigmoid()
        )

    def forward(self, x):
        spatial_feat = self.vss(x)
        channel_weights = self.channel_attn(
            spatial_feat.permute(0, 3, 1, 2)
        ).permute(0, 2, 3, 1)
        return spatial_feat * channel_weights + x



def discretize_ssm(delta, A, B, C):
